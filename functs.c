...
float filter(float h2o_in, float nacl_in);
...
float filter(float h2o_in, float nacl_concentrate){
  return ((h2o_in - (nacl_concentrate / 1000))*11)/12;
}
